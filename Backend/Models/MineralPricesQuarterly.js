...
const getAllQuarterlyPrices = async () => {
  const query = `SELECT * FROM mineral_prices_quarterly WHERE deleted_at IS NULL ORDER BY price_date DESC`;
  const result = await pool.query(query);
  return result.rows;
};

const getQuarterlyPriceById = async (id) => {
  const query = `SELECT * FROM mineral_prices_quarterly WHERE id = $1 AND deleted_at IS NULL`;
  const result = await pool.query(query, [id]);
  return result.rows[0];
};

const createQuarterlyPrice = async ({ mineral_production_id, price, max_price, min_price, avg_price, avg_change, avg_value_change, price_date }) => {
  const query = `
    INSERT INTO mineral_prices_quarterly (mineral_production_id, price, max_price, min_price, avg_price, avg_change, avg_value_change, price_date)
...
const deleteQuarterlyPrice = async (id) => {
  const query = `
    UPDATE mineral_prices_quarterly
