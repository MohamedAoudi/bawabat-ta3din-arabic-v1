...
const createCountry = async ({ name_ar, name_en, name_fr, iso_code, display_order = 0 }) => {
  const result = await pool.query(
    ...
    [name_ar, name_en, name_fr, iso_code, display_order]
  );
  return result.rows[0];
};

const updateCountry = async (id, { name_ar, name_en, name_fr, iso_code, display_order }) => {
  const result = await pool.query(
    `UPDATE countries
     ...
     WHERE id = $6
     RETURNING id, name_ar, name_en, name_fr, iso_code, display_order, created_at, updated_at`,
    [name_ar, name_en, name_fr, iso_code, display_order, id]
  );
  return result.rows[0];
};
