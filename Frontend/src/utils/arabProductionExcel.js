...
export const ARAB_PRODUCTION_EXCEL_KEYS = [
  "country",
  "mineral_hs",
  "year",
  "production_value",
...
const HEADER_ALIASES = {
  country: ["country", "pays", "الدولة", "country name", "nom pays"],
...
function buildHeaderRow(t) {
  return ARAB_PRODUCTION_EXCEL_KEYS.map((key) => t.excelFields?.[key] || t.fields?.[key] || key);
}
...
function findCountry(countries, value) {
  ...
  return (
    countries.find((country) =>
      [country.name_ar, country.name_en, country.name_fr, country.iso_code].some((name) => normalizeHeaderKey(name) === nq)
    ) || null
  );
}
...
function ArabProductionToRow(row, minerals, t) {
  const headers = buildHeaderRow(t);
  const country = row.country;
  const mineral = minerals.find((m) => String(m.id) === String(row.mineral_production_id));
  const values = {
    country: countryName(country) || row.country_name_en || row.country_name_fr || row.country_name_ar || row.country_id || "",
  ...
  const output = {};
  ARAB_PRODUCTION_EXCEL_KEYS.forEach((key, index) => {
    output[headers[index]] = values[key] ?? "";
  });
  return output;
}

export function exportArabProductionExcel(rows, countries, minerals, t) {
  const headers = buildHeaderRow(t);
  const dataRows =
    rows.length > 0
      ? rows.map((row) => {
          const country = countries.find((c) => String(c.id) === String(row.country_id));
          return ArabProductionToRow({ ...row, country }, minerals, t);
        })
      : [Object.fromEntries(headers.map((header) => [header, ""]))];
  const ws = buildStyledSheet(dataRows, headers);
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, "Arab production");
  downloadWorkbook(wb, "ARAB_PRODUCTION.xlsx");
}

export function exportArabProductionTemplateExcel(t, countries = [], minerals = []) {
  const headers = buildHeaderRow(t);
  const demo = {
    ...DEMO_ROW,
    country: countryName(countries[0]) || DEMO_ROW.country,
    mineral_hs: mineralHs(minerals[0]) || DEMO_ROW.mineral_hs,
  };
  const row = {};
  ARAB_PRODUCTION_EXCEL_KEYS.forEach((key, index) => {
  ...
  downloadWorkbook(wb, "ARAB_PRODUCTION_template.xlsx");
}

export async function parseArabProductionExcelFile(file, countries, minerals, t) {
  const rawRows = await readExcelRows(file);
  if (!rawRows.length) return [];

  const translatedFields = { ...t.excelFields, ...t.fields };
  const columnMap = buildColumnMap(Object.keys(rawRows[0]), ARAB_PRODUCTION_EXCEL_KEYS, HEADER_ALIASES, translatedFields);

  const payloads = [];
  for (const raw of rawRows) {
    const item = rowToRecord(raw, columnMap);
    if (!item.country && !item.mineral_hs && !item.year && !item.production_value && !item.production_value_base) continue;

    const country = findCountry(countries, item.country);
...
export function ArabProductionRecordKey(payload) {
  return `${Number(payload.country_id)}|${Number(payload.mineral_production_id)}|${Number(payload.year)}`;
}

export function dedupeArabProductionPayloads(payloads) {
  const map = new Map();
  for (const payload of payloads) {
    map.set(ArabProductionRecordKey(payload), payload);
  }
  return [...map.values()];
}

export function findMatchingArabProduction(rows, payload) {
  return (
    rows.find(
      (row) =>
        Number(row.mineral_production_id) === Number(payload.mineral_production_id) &&
        Number(row.country_id) === Number(payload.country_id) &&
