...
/* ════════════════════════════════════════════════════════
   MINI CHARTS — prop "active" drives all animations
   No CSS class dependency → works on any background
════════════════════════════════════════════════════════ */

const MiniBarChart = ({ active }) => {
  const data = [
    { v: 55 }, { v: 72 }, { v: 38 }, { v: 63 }, { v: 78 },
    { v: 22 }, { v: 95 }, { v: 48 }, { v: 41 }, { v: 18 },
    { v: 67 }, { v: 59 }, { v: 15 }, { v: 44 }, { v: 36 },
    { v: 29 }, { v: 52 }, { v: 83 }, { v: 88 }, { v: 71 }, { v: 33 },
  ];
  return (
    <div style={{
      height: 90, width: "100%",
      background: "rgba(255,255,255,0.06)",
      borderRadius: 8, padding: "10px 6px 4px",
...
const MiniLineChart = ({ active }) => {
  const pts = [
    [4,76],[18,72],[32,70],[46,67],[60,65],[74,63],
    [88,60],[102,56],[116,14],[130,40],[144,66],[158,42],[172,44],[186,38],
  ];
  ...
  return (
    <div style={{
      ...
      <svg viewBox="0 0 190 88" style={{ width: "100%", height: "100%", overflow: "visible" }}>
        {[20, 40, 60, 80].map(y => (
          <line key={y} x1="0" y1={y} x2="190" y2={y} stroke="rgba(255,255,255,0.07)" strokeWidth="1" />
        ))}
        <path
          d={path} fill="none" stroke="#7ee0c0" strokeWidth="2.5"
        ...
        {pts.map((p, i) => (
          <circle
            key={i} cx={p[0]} cy={p[1]} r="2.5" fill="#c9a84c"
            opacity={active ? 1 : 0}
            style={{
...
const MiniGroupedBar = ({ active }) => {
  ...
  return (
    <div style={{
      height: 90, width: "100%",
      background: "rgba(255,255,255,0.06)",
      borderRadius: 8, padding: "10px 8px 4px",
      display: "flex", alignItems: "flex-end", gap: 2,
    }}>
      {a.map((v, i) => (
        <div key={i} style={{
          flex: 1, display: "flex", alignItems: "flex-end",
          justifyContent: "center", gap: 1, height: "100%",
        }}>
          <div style={{
            width: "45%", borderRadius: "2px 2px 0 0",
            background: "rgba(255,255,255,0.55)",
            height: active ? `${v}%` : "0%",
            opacity: active ? 1 : 0.2,
            transition: active
              ? `height 700ms cubic-bezier(.16,1,.3,1) ${i * 38}ms, opacity 350ms ease ${i * 38}ms`
              : "height 250ms ease, opacity 250ms ease",
          }} />
          <div style={{
            width: "45%", borderRadius: "2px 2px 0 0",
            ...
            opacity: active ? 1 : 0.2,
            transition: active
              ? `height 700ms cubic-bezier(.16,1,.3,1) ${i * 38 + 75}ms, opacity 350ms ease ${i * 38 + 75}ms`
              : "height 250ms ease, opacity 250ms ease",
          }} />
        </div>
      ))}
...
const MiniDonut = ({ active }) => (
  <div style={{
    ...
    <div style={{
      ...
      transition: active
        ? "opacity 550ms ease 80ms, transform 700ms cubic-bezier(.16,1,.3,1) 80ms"
...
/* ════════════════════════════════════════════════════════
   INDICATOR ROW CARD — owns its hover state
   passes it as "active" prop directly to chart
════════════════════════════════════════════════════════ */
      ...
      <div style={{ display: "flex", gap: 20, alignItems: "stretch", flexWrap: "wrap" }}>
        ...
        <div style={{ flex: "1 1 280px", display: "flex", gap: 16, alignItems: "flex-start" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ fontSize: "0.82rem", color: "rgba(255,255,255,0.5)", lineHeight: 1.75, margin: "0 0 14px" }}>
              {card.desc}
            </p>
            <a href={card.href} style={{
              ...
              padding: "6px 18px",
              border: `1px solid ${card.tagColor}40`,
              borderRadius: 2, fontSize: "0.76rem", fontWeight: 700,
              color: card.tagColor, letterSpacing: "0.04em", textDecoration: "none",
        ...
        {/* Right — mini chart, active = hovered */}
  ...
  return (
    <div className="min-h-screen" dir="rtl" lang="ar"
      ...
      <style>{`
        ...
        :root {
          --forest: #082721; --forest-mid: #0d3d34;
          --gold: #c9a84c; --gold-light: #e8d08a; --gold-pale: #f7f0dc;
          --cream: #f5f3ef; --parchment: #ede9df;
          --ink: #1a1510; --muted: #7a7060;
        }
        ...
        .reveal.is-visible.d1{animation-delay:60ms} .reveal.is-visible.d2{animation-delay:130ms}
        .reveal.is-visible.d3{animation-delay:200ms} .reveal.is-visible.d4{animation-delay:280ms}
        ...
        .search-container {
          background:rgba(255,255,255,0.83); backdrop-filter:blur(20px);
          border:1px solid rgba(201,168,76,0.35);
          transition:all 0.4s cubic-bezier(.16,1,.3,1);
        }
        .search-container.focused {
          background:rgb(255,255,255); border-color:rgba(201,168,76,0.7);
        ...
        .kpi-card { background:var(--forest); border:1px solid rgba(201,168,76,0.2); transition:transform 0.4s cubic-bezier(.16,1,.3,1),box-shadow 0.4s,border-color 0.4s; }
        .kpi-card:hover { transform:translateY(-10px); border-color:rgba(201,168,76,0.6); box-shadow:0 30px 60px rgba(8,39,33,0.35),0 0 0 1px rgba(201,168,76,0.3); }
        .kpi-number { font-size:2.8rem; font-weight:900; letter-spacing:-0.02em; line-height:1; }

        .section-label { display:inline-flex; align-items:center; gap:8px; padding:6px 16px; background:var(--forest); color:var(--gold); border-radius:2px; font-size:0.8rem; font-weight:700; letter-spacing:0.08em; }

        .ind-link { display:inline-flex; align-items:center; gap:6px; padding:6px 18px; border:1px solid rgba(8,39,33,0.2); border-radius:2px; font-size:0.78rem; font-weight:700; color:var(--forest); letter-spacing:0.04em; transition:background 0.25s,border-color 0.25s,color 0.25s; }
        .ind-link:hover { background:var(--forest); color:var(--gold); border-color:var(--forest); }

        .trade-card { background:linear-gradient(135deg,var(--forest) 0%,#0d3d34 100%); border:1px solid rgba(201,168,76,0.2); transition:transform 0.35s cubic-bezier(.16,1,.3,1),box-shadow 0.35s,border-color 0.35s; }
        .trade-card:hover { transform:translateY(-8px); border-color:rgba(201,168,76,0.5); box-shadow:0 24px 48px rgba(8,39,33,0.3); }

        .reserve-card { background:white; transition:transform 0.35s cubic-bezier(.16,1,.3,1),box-shadow 0.35s; }
        .reserve-card:hover { transform:translateY(-8px); box-shadow:0 24px 48px rgba(8,39,33,0.12); }

        .countries-wrap { background:white; border:1px solid rgba(8,39,33,0.08); }
      ...
      <header className="relative overflow-hidden" style={{ height: "100vh", minHeight: 600, maxHeight: 800, marginTop: -80 }}>
        <video className="absolute inset-0 h-full w-full object-cover" src={bgHeaderVideo} autoPlay loop muted playsInline />
        ...
        <div className="relative z-10 h-full flex flex-col items-center justify-center text-center px-6">
          <div className="hero-title mb-6">
            <span style={{ display:"inline-block", background:"rgba(201,168,76,0.15)", border:"1px solid rgba(201,168,76,0.4)", color:"var(--gold-light)", padding:"4px 20px", fontSize:"0.75rem", fontWeight:700, letterSpacing:"0.15em", borderRadius:"2px", marginBottom:20, textTransform:"uppercase" }}>
              ✦ المنظومة التعدينية العربية ✦
            </span>
          </div>

          <h1 className="hero-title text-white" style={{ fontSize:"clamp(2rem,5vw,4rem)", fontWeight:900, lineHeight:1.15, letterSpacing:"-0.02em", textShadow:"0 4px 40px rgba(0,0,0,0.4)", maxWidth:800, marginBottom:"1rem" }}>
            بوابة المؤشرات <span className="text-shimmer">التعدينية العربية</span>
          </h1>

          <p className="hero-sub" style={{ color:"rgba(255,255,255,0.72)", fontSize:"clamp(0.9rem,1.8vw,1.15rem)", maxWidth:560, lineHeight:1.8, marginBottom:"2.5rem" }}>
          ...
          <div className="hero-search w-full" style={{ maxWidth: 580 }}>
            <div className={`search-container flex items-center gap-3 rounded-sm px-5 py-3 ${searchFocused ? "focused" : ""}`}>
              <button type="button" style={{ background:"linear-gradient(135deg,#c9a84c,#e8d08a)", color:"#fff", padding:"8px 22px", borderRadius:"2px", fontSize:"0.82rem", fontWeight:800, letterSpacing:"0.04em", whiteSpace:"nowrap", border:"none", cursor:"pointer" }}>
                بحث ذكي
              </button>
              <input type="text" placeholder="ابحث عن معدن، دولة، أو إحصائية..."
                onFocus={() => setSearchFocused(true)} onBlur={() => setSearchFocused(false)}
                style={{ flex:1, background:"transparent", border:"none", outline:"none", color:"black", fontSize:"0.9rem", textAlign:"right", caretColor:"black" }}
              />
      ...
      <main style={{ maxWidth: 1400, margin: "0 auto", padding: "0 24px 80px" }}>
          ...
          <div style={{
            ...
            <div style={{ position:"relative", zIndex:1, display:"flex", justifyContent:"space-between", alignItems:"flex-end", marginBottom:32, flexWrap:"wrap", gap:12 }}>
              <div>
                ...
                <h3 style={{ fontSize:"1.6rem", fontWeight:900, color:"white", marginTop:12, marginBottom:4 }}><span className="text-shimmer">المؤشرات التعدينية</span></h3>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:14, position:"relative", zIndex:1 }}>
              {indicatorCards.map((card, i) => (
                <IndicatorRowCard key={i} card={card} />
              ))}
            </div>
        ...
        <section className="reveal d3" style={{ marginTop: 72 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end", marginBottom:28, flexWrap:"wrap", gap:12 }}>
            ...
            <a href="/trade-indicators" className="ind-link"><i className="fas fa-arrow-left" /> جميع مؤشرات التجارة</a>
          </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))", gap:20 }}>
            {[
              ...
              <div key={i} className="trade-card rounded-sm p-6" style={{ borderRadius:4 }}>
                ...
                <a href={card.href} style={{ display:"inline-flex", alignItems:"center", gap:6, padding:"6px 18px", background:"rgba(201,168,76,0.12)", border:"1px solid rgba(201,168,76,0.3)", borderRadius:2, fontSize:"0.78rem", fontWeight:700, color:"var(--gold)", letterSpacing:"0.04em", textDecoration:"none" }}>
                  <i className="fas fa-arrow-left" /> المزيد
                </a>
              </div>
            ))}
            ...
            {[
              ...
              <div key={i} className="reserve-card rounded-sm p-6" style={{ borderRadius:4, borderTop:`3px solid ${card.accent}` }}>
                <div style={{ display:"flex", gap:14, alignItems:"flex-start", marginBottom:16 }}>
                  <div style={{ width:44, height:44, background:i===0?"rgba(201,168,76,0.1)":"rgba(8,39,33,0.08)", borderRadius:10, display:"flex", alignItems:"center", justifyContent:"center", color:card.accent, fontSize:"1rem", flexShrink:0 }}>
                    <i className={`fas ${card.icon}`} />
                  </div>
                  <div>
        ...
        <section className="reveal d4" style={{ marginTop: 72 }}>
          <div className="countries-wrap rounded-sm p-8" style={{ borderRadius:4 }}>
            ...
            <div className="gold-divider" style={{ marginBottom: 28 }} />
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(90px,1fr))", gap:"20px 16px" }}>
              {countries.map((c) => (
            ...
            <div style={{ marginTop:20, paddingTop:16, borderTop:"1px solid rgba(8,39,33,0.08)", fontSize:"0.8rem", color:"var(--muted)" }}>
              الدولة المختارة: <span style={{ fontWeight:800, color:"var(--forest)" }}>{selectedCountry}</span>
            </div>
          </div>
        </section>
          ...
          <div style={{ background:"white", border:"1px solid rgba(8,39,33,0.08)", borderRadius:4, padding:32 }}>
            ...
            <div style={{ display:"flex", justifyContent:"center", gap:6, marginTop:16 }}>
              {sponsorSlides.map((_, i) => (
                <button key={i} onClick={() => setSponsorSlide(i)} style={{ width:i===sponsorSlide?20:6, height:6, borderRadius:3, border:"none", cursor:"pointer", background:i===sponsorSlide?"var(--gold)":"rgba(8,39,33,0.15)", transition:"all 0.3s" }} />
              ))}
            </div>
          </div>
        </section>
          ...
          <div className="chat-cta rounded-sm p-10 text-center" style={{ borderRadius:4 }}>
            <div style={{ position:"relative", zIndex:1 }}>
              ...
              <h2 style={{ fontSize:"1.8rem", fontWeight:900, color:"white", marginBottom:12 }}>
                دردشة مع <span className="text-shimmer">المساعد الذكي</span>
              </h2>
