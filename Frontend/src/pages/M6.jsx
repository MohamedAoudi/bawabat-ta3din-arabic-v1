  ...
  for (let i = 1; i < lines.length; i += 1) {
    ...
    if (headerLike) {
      idx = {
        ...
        partner: normalized.indexOf("partner"),
    ...
    if (String(flowRaw).toLowerCase() !== "import") continue;
    ...
    const country =
      (idx.reporter !== -1 ? cols[idx.reporter] : cols[0]) ||
      (idx.partner !== -1 ? cols[idx.partner] : "");
...
function extractAllCountriesFromCsv(csvText) {
  const lines = String(csvText || "").split(/\r?\n/);
  ...
  for (let i = 1; i < lines.length; i += 1) {
    ...
    const normalized = cols.map(normalizeHeaderKey);
    const headerLike = normalized.includes("reporter") && normalized.includes("flow");
...
export default function M6Page() {
  ...
  const fullCountryPack = useMemo(() => {
    if (!countryPack) return null;

    const map = new Map((countryPack.table || []).map((r) => [r.c, r.v]));
    const table = barCountries
      .map((c) => ({ c, v: Number((map.get(c) || 0).toFixed(2)) }))
      .sort((a, b) => b.v - a.v);

    return { table };
  }, [countryPack, barCountries]);
  ...
  }, [selectedKeys, unit, importCountryData, barYears, effectiveBarCountries]);

  useEffect(() => {
    const ctx = donutCanvasRef.current?.getContext("2d");
    if (!ctx || !fullCountryPack) return;
    ...
    const rows = fullCountryPack.table || [];
  ...
  }, [fullCountryPack, unit, effectiveCountry]);
      ...
      <main className="min-h-screen py-6 sm:py-8">
        ...
        <header className="mb-6 rounded-3xl bg-gradient-to-l from-[#082721] to-[#051712] px-6 py-8 text-center text-white shadow-lg ring-1 ring-[#ddbc6b]/25">
          ...
          <p className="text-sm text-slate-100/80">
            مقارنة زمنية لإجمالي واردات المعادن حسب السنة.
          </p>
        </header>
            ...
            <div className="rounded-3xl bg-white/95 p-4 shadow-lg shadow-slate-900/10 ring-1 ring-slate-200/70">
              <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                <div className="text-base font-extrabold text-slate-800">
                 اجمالي الواردات التعدينية للدول حسب السنة والخام والمنتج
                </div>
